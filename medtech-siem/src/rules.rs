...
// Règle : Si un même source_name a 5 échecs de login en moins d'une minute, déclencher une alerte
    // fenêtre de 1 minute
    let one_minute_ago = Utc::now() - Duration::minutes(1);

    // Compter les login_failed récents
    ...
    if count.0 >= 5 {
        ...
        // Vérifier si une alerte existe déjà dans la dernière minute
        ...
        if existing_alert.0 == 0 {
            ...
            println!("🚨 Nouvelle alerte créée pour {}", source_name);
